...
class CarlaSpawnObjects(object):
    ...
    def setup_vehicles(self, vehicles):
        for vehicle in vehicles:
            if self.spawn_sensors_only is True:
                ...
                spawn_point_param = vehicle["spawn_point"]
    ...
    def check_spawn_point_param(self, spawn_point_parameter):
        if len(spawn_point_parameter.keys()) != 6:
            rospy.logwarn("Invalid spawnpoint '{}'".format(spawn_point_parameter))
            return None
        spawn_point = self.create_spawn_point(
            float(spawn_point_parameter["x"]),
            float(spawn_point_parameter["y"]),
            float(spawn_point_parameter["z"]),
            float(spawn_point_parameter["roll"]),
            float(spawn_point_parameter["pitch"]),
            float(spawn_point_parameter["yaw"])
        )
        return spawn_point
